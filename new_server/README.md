...
- Phân quyền theo `domain` hoặc `mailbox`, không theo account kiểu cũ
...
- `GET|POST /v1/domains/:domain/permissions`
- `GET|PATCH|DELETE /v1/domains/:domain/permissions/:permissionId`
...
- `GET|POST|PUT /v1/groups/:groupId/emails`
...
- `GET /v1/domains` không đại diện đầy đủ scope của mailbox-only user
- `GET /v1/emails`, `GET /v1/inboxes/:emailAddress` và `POST /v1/emails/batch` của user thường đều bị chặn bởi `email_registers`
